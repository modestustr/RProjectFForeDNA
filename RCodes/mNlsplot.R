mNlsplot <-
  function(data, model=1, start=c(a=1,b=1,c=1,d=1,e=1), xlab="Explanatory Variable", 
           ylab="Response Variable", title="Title", position=1)
  {
    ...
    plot_raw_data <- function(data) {
      points(data[, 1], data[, 2], col = "orange", pch = 1)
      title(main = title)
    }
    ...
    plot2=function(data)
    {
      ...
      plot(datao[,2]~as.numeric(as.character(datao[,1])), data=datao, xlab = xlab,
           ylab = ylab, col="white", bty="n",xlim = c(minx,
    ...
    # lll = list(plot1,plot2 )  # Add plot_raw_data function to the list
    # lll[[ppp]](data)
    # print(ppp)

    # Initialize the plot
    # plot(data[, 1], data[, 2], col = "dark red", type = "l", lty = 1, xlab = xlab, ylab = ylab, main = title)
    
    # Add the model-based prediction plot
    plot2(data)
    
    # Add the raw data points plot
    plot_raw_data(data)
    
    # Create the legend
    # legend(t[position], legend = eee[[model]], bty = "n", cex = 0.8, col = c("dark red", "dark blue", "green"))
